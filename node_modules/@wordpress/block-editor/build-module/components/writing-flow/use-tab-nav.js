...
export default function useTabNav() {
  ...
  const isNavigationMode = useSelect(select => select(blockEditorStore).isNavigationMode(), []); // Don't allow tabbing to this element in Navigation mode.
  ...
  const ref = useRefEffect(node => {
    function onKeyDown(event) {
      ...
      if ((isFormElement(event.target) || event.target.getAttribute('data-block') === getSelectedBlockClientId()) && isFormElement(focus.tabbable[direction](event.target))) {
        return;
      }
    ...
    function preventScrollOnTab(event) {
      ...
      const isShift = event.shiftKey;
      const direction = isShift ? 'findPrevious' : 'findNext';
      const target = focus.tabbable[direction](event.target); // Only do something when the next tabbable is a focus capture div (before/after)
