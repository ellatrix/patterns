...
import { store as reusableBlocksStore } from '@wordpress/reusable-blocks'; // Disable Reason: Needs to be refactored.
...
const BlockActionsMenu = _ref => {
  ...
  const clipboardBlock = clipboard && rawHandler({
    HTML: clipboard
  })[0];
  const isPasteEnabled = clipboardBlock && canInsertBlockType(clipboardBlock.name, rootClientId);
  const innerBlockCount = useSelect(select => select(blockEditorStore).getBlockCount(selectedBlockClientId), [selectedBlockClientId]);
  const {
    actionTitle: {
  ...
  const allOptions = {
    ...
    copyButton: {
      ...
      onSelect: () => {
        const serializedBlock = serialize(getBlocksByClientId(selectedBlockClientId));
    ...
    convertToRegularBlocks: {
      ...
      onSelect: () => {
        ...
        createSuccessNotice(sprintf(successNotice, (reusableBlock === null || reusableBlock === void 0 ? void 0 : (_reusableBlock$title = reusableBlock.title) === null || _reusableBlock$title === void 0 ? void 0 : _reusableBlock$title.raw) || blockTitle));
        convertToRegularBlocks();
      }
    }
  };
...
export default compose(withSelect((select, _ref2) => {
  ...
  const reusableBlock = isReusableBlockType ? select(coreStore).getEntityRecord('postType', 'wp_block', block === null || block === void 0 ? void 0 : block.attributes.ref) : undefined;
  return {
    blockTitle,
    canInsertBlockType,
...
}), withDispatch((dispatch, _ref3, _ref4) => {
  let {
    clientId,
    rootClientId,
    currentIndex,
  ...
  return {
    ...
    onMoveDown: function () {
      ...
      return moveBlocksDown([clientId], rootClientId, ...args);
    },
    onMoveUp: function () {
      ...
      return moveBlocksUp([clientId], rootClientId, ...args);
    },
