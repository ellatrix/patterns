...
import { addActiveFormats, getMultilineTag, getAllowedFormats, isShortcode, createLinkInParagraph } from './utils';
...
function RichTextWrapper(_ref, forwardedRef) {
  ...
  const splitValue = useCallback(function (record) {
    ...
    const isAfterOriginal = isEmpty(before) && !isEmpty(after); // Create a block with the content before the caret if there's no pasted
  ...
  const onEnter = useCallback(_ref3 => {
    let {
      value,
      onChange,
      shiftKey
    ...
    if (onReplace) {
      const transforms = getBlockTransforms('from').filter(_ref4 => {
        let {
          type
        } = _ref4;
        return type === 'enter';
  ...
  const onPaste = useCallback(_ref5 => {
    let {
      value,
      onChange,
      html,
    ...
    const presentEmbedHandlerPicker = () => {
      ...
      return (_embedHandlerPickerRe = embedHandlerPickerRef.current) === null || _embedHandlerPickerRe === void 0 ? void 0 : _embedHandlerPickerRe.presentPicker({
        ...
        createLink: () => createLinkInParagraph(plainText.trim(), onReplace)
      });
    };
    ...
    if (typeof content === 'string') {
      ...
      addActiveFormats(valueToInsert, activeFormats); // If the content should be multiline, we should process text
  ...
  const inputRule = useCallback((value, valueToFormat) => {
    ...
    const transformation = findTransform(prefixTransforms, _ref7 => {
      let {
        prefix
      } = _ref7;
