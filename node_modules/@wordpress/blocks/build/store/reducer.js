...
function blockStyles() {
  ...
  switch (action.type) {
    case 'ADD_BLOCK_TYPES':
      return { ...state,
        ...(0, _lodash.mapValues)(keyBlockTypesByName(action.blockTypes), blockType => getUniqueItemsByName([...(0, _lodash.get)(blockType, ['styles'], []).map(style => ({ ...style,
          source: 'block'
        })), ...(0, _lodash.get)(state, [blockType.name], []).filter(_ref => {
          let {
            source
          } = _ref;
          return 'block' !== source;
    ...
    case 'ADD_BLOCK_STYLES':
      return { ...state,
        [action.blockName]: getUniqueItemsByName([...(0, _lodash.get)(state, [action.blockName], []), ...action.styles])
      };

    case 'REMOVE_BLOCK_STYLES':
      return { ...state,
        [action.blockName]: (0, _lodash.get)(state, [action.blockName], []).filter(style => action.styleNames.indexOf(style.name) === -1)
      };
  }
...
function blockVariations() {
  ...
  switch (action.type) {
    case 'ADD_BLOCK_TYPES':
      return { ...state,
        ...(0, _lodash.mapValues)(keyBlockTypesByName(action.blockTypes), blockType => {
          return getUniqueItemsByName([...(0, _lodash.get)(blockType, ['variations'], []).map(variation => ({ ...variation,
            source: 'block'
          })), ...(0, _lodash.get)(state, [blockType.name], []).filter(_ref2 => {
            let {
              source
            } = _ref2;
            return 'block' !== source;
    ...
    case 'ADD_BLOCK_VARIATIONS':
      return { ...state,
        [action.blockName]: getUniqueItemsByName([...(0, _lodash.get)(state, [action.blockName], []), ...action.variations])
      };

    case 'REMOVE_BLOCK_VARIATIONS':
      return { ...state,
        [action.blockName]: (0, _lodash.get)(state, [action.blockName], []).filter(variation => action.variationNames.indexOf(variation.name) === -1)
      };
  }
