...
export function blockStyles() {
  ...
  switch (action.type) {
    case 'ADD_BLOCK_TYPES':
      return { ...state,
        ...mapValues(keyBlockTypesByName(action.blockTypes), blockType => getUniqueItemsByName([...get(blockType, ['styles'], []).map(style => ({ ...style,
          source: 'block'
        })), ...get(state, [blockType.name], []).filter(_ref => {
          let {
            source
          } = _ref;
          return 'block' !== source;
    ...
    case 'ADD_BLOCK_STYLES':
      return { ...state,
        [action.blockName]: getUniqueItemsByName([...get(state, [action.blockName], []), ...action.styles])
      };

    case 'REMOVE_BLOCK_STYLES':
      return { ...state,
        [action.blockName]: get(state, [action.blockName], []).filter(style => action.styleNames.indexOf(style.name) === -1)
      };
  }
...
export function blockVariations() {
  ...
  switch (action.type) {
    case 'ADD_BLOCK_TYPES':
      return { ...state,
        ...mapValues(keyBlockTypesByName(action.blockTypes), blockType => {
          return getUniqueItemsByName([...get(blockType, ['variations'], []).map(variation => ({ ...variation,
            source: 'block'
          })), ...get(state, [blockType.name], []).filter(_ref2 => {
            let {
              source
            } = _ref2;
            return 'block' !== source;
    ...
    case 'ADD_BLOCK_VARIATIONS':
      return { ...state,
        [action.blockName]: getUniqueItemsByName([...get(state, [action.blockName], []), ...action.variations])
      };

    case 'REMOVE_BLOCK_VARIATIONS':
      return { ...state,
        [action.blockName]: get(state, [action.blockName], []).filter(variation => action.variationNames.indexOf(variation.name) === -1)
      };
  }
