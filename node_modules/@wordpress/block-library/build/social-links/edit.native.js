 ...
 */
const ALLOWED_BLOCKS = _variations.default.map(v => `core/social-link-${v.name}`); // Template contains the links that show when start.
...
function SocialLinksEdit(_ref) {
  ...
  const renderFooterAppender = (0, _element.useRef)(() => (0, _element.createElement)(_reactNative.View, null, (0, _element.createElement)(_blockEditor.InnerBlocks.ButtonBlockAppender, {
    isFloating: true
  })));
  ...
  function renderPlaceholder() {
    return [...new Array(innerBlocks.length || 1)].map((_, index) => (0, _element.createElement)(_reactNative.View, {
...
var _default = (0, _compose.compose)((0, _data.withSelect)((select, _ref2) => {
  ...
  const activeInnerBlocks = innerBlocks.filter(block => {
