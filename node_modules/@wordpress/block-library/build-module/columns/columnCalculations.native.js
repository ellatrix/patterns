 ...
 */
import { ALIGNMENT_BREAKPOINTS, convertUnitToMobile } from '@wordpress/components';
/**
...
const MIN_WIDTH = (_styles$columnsContai = styles.columnsContainer) === null || _styles$columnsContai === void 0 ? void 0 : _styles$columnsContai.minWidth;
/**
...
const MARGIN = (_styles$columnsContai2 = styles.columnsContainer) === null || _styles$columnsContai2 === void 0 ? void 0 : _styles$columnsContai2.marginLeft;
...
export const calculateContainerWidth = (containerWidth, columnsInRow) => 2 * MARGIN + containerWidth - columnsInRow * 2 * MARGIN;
export const getContentWidths = (columnsInRow, width, columnCount, innerColumns, globalStyles) => {
  const widths = {};
  const columnWidthsWithUnits = getWidths(innerColumns, false);
  const columnWidths = getColumnWidths(innerColumns, columnCount); // Array of column width attribute values

  const columnWidthsValues = columnWidthsWithUnits.map(v => convertUnitToMobile({
    width
  }, globalStyles, v)); // The sum of column width attribute values

  const columnWidthsSum = columnWidthsValues.reduce((acc, curr) => acc + curr, 0); // Array of ratios of each column width attribute value to their sum

  const columnRatios = columnWidthsValues.map(colWidth => colWidth / columnWidthsSum); // Array of calculated column width for its ratio

  const columnWidthsPerRatio = columnRatios.map(columnRatio => columnRatio * calculateContainerWidth(width, columnsInRow)); //  Array of columns whose calculated width is lower than minimum width value

  const filteredColumnWidthsPerRatio = columnWidthsPerRatio.filter(columnWidthPerRatio => columnWidthPerRatio <= MIN_WIDTH); // Container width to be divided. If there are some results within `filteredColumnWidthsPerRatio`
  ...
  const baseContainerWidth = width - filteredColumnWidthsPerRatio.length * MIN_WIDTH; // The minimum percentage ratio for which column width is equal minimum width value

  const minPercentageRatio = MIN_WIDTH / calculateContainerWidth(width, columnsInRow); // The sum of column widths which ratio is higher than `minPercentageRatio`
