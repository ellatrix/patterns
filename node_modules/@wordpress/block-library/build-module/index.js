 ...
 */
import { setDefaultBlockName, setFreeformContentHandlerName, setUnregisteredTypeHandlerName, setGroupingBlockName } from '@wordpress/blocks';
/**
...
export const __experimentalGetCoreBlocks = () => getAllBlocks().filter(_ref => {
  let {
    metadata
  } = _ref;
  return !isBlockMetadataExperimental(metadata);
});
/**
...
export const registerCoreBlocks = function () {
...
export const __experimentalRegisterExperimentalCoreBlocks = process.env.IS_GUTENBERG_PLUGIN ? function () {
  let {
    enableFSEBlocks
  } = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  const enabledExperiments = [enableFSEBlocks ? 'fse' : null];
  getAllBlocks().filter(_ref3 => {
    let {
      metadata
    } = _ref3;
    return isBlockMetadataExperimental(metadata);
    ...
    return __experimental === true || enabledExperiments.includes(__experimental);
  }).forEach(_ref5 => {
    let {
      init
    } = _ref5;
