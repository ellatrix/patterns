...
export function insertRow(state, _ref3) {
  let {
    sectionName,
    rowIndex,
    columnCount
  ...
  const cellCount = columnCount === undefined ? get(firstRow, ['cells', 'length']) : columnCount; // Bail early if the function cannot determine how many cells to add.
...
export function deleteRow(state, _ref5) {
  let {
    sectionName,
    rowIndex
  } = _ref5;
...
export function deleteColumn(state, _ref8) {
  let {
    columnIndex
  } = _ref8;
...
export function toggleSection(state, sectionName) {
  ...
  const columnCount = get(state, ['body', 0, 'cells', 'length'], 1); // Section doesn't exist, insert an empty row to create the section.
