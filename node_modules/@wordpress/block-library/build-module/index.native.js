...
export const coreBlocks = [// Common blocks are grouped at the top to prioritize their display
...
const registerBlockVariations = block => {
  ...
  [...settings.variations].sort((a, b) => a.title.localeCompare(b.title)).forEach(v => {
    registerBlockType(`${name}-${v.name}`, { ...metadata,
      name: `${name}-${v.name}`,
      ...settings,
      icon: v.icon(),
      title: v.title,
...
const iOSOnly = block => Platform.OS === 'ios' ? block : devOnly(block); // Hide the Classic block and SocialLink block


addFilter('blocks.registerBlockType', 'core/react-native-editor', (settings, name) => {
