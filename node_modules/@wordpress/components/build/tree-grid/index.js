...
ref) {
  ...
  const onKeyDown = (0, _element.useCallback)(event => {
    ...
    if (hasModifierKeyPressed || ![_keycodes.UP, _keycodes.DOWN, _keycodes.LEFT, _keycodes.RIGHT, _keycodes.HOME, _keycodes.END].includes(keyCode)) {
      return;
    ...
    if ([_keycodes.LEFT, _keycodes.RIGHT].includes(keyCode)) {
      ...
      event.preventDefault();
    } else if ([_keycodes.UP, _keycodes.DOWN].includes(keyCode)) {
      ...
      event.preventDefault();
    } else if ([_keycodes.HOME, _keycodes.END].includes(keyCode)) {
      // Calculate the rowIndex of the next row.
      const rows = Array.from(treeGridElement.querySelectorAll('[role="row"]'));
      const currentRowIndex = rows.indexOf(activeRow);
      let nextRowIndex;
